...
public class DynamicLatexSetsMenu extends DynamicMenuBar {
   ...
   */
  public static DynamicLatexSetsMenu get(HasCommandHandlers commandSource) {
    if (instance == null) {
      instance = new DynamicLatexSetsMenu(commandSource);
    }
    return instance;
  }
   ...
   * @param commandSource the command source
   */
  protected DynamicLatexSetsMenu(HasCommandHandlers commandSource) {
    ...
    GWT.runAsync(new RunAsyncCallback() {
		...
		public void onSuccess() {
		  ...
		  for (int i=0; i<cmds.length; i++) {
			...
			items[i] = DynamicLatexSetsMenu.this.createItem(cmd.getIcon(), cmd.getTitle(), new SystemPasteCommand(cmd.getText()));
		  }
