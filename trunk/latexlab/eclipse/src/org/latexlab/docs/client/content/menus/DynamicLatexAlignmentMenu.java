...
public class DynamicLatexAlignmentMenu extends DynamicMenuBar {
   ...
   */
  public static DynamicLatexAlignmentMenu get(HasCommandHandlers commandSource) {
    if (instance == null) {
      instance = new DynamicLatexAlignmentMenu(commandSource);
    }
    return instance;
  }
   ...
   * @param commandSource the command source
   */
  protected DynamicLatexAlignmentMenu(HasCommandHandlers commandSource) {
    ...
    GWT.runAsync(new RunAsyncCallback() {
		...
		public void onSuccess() {
		  ...
		  for (int i=0; i<cmds.length; i++) {
			...
			items[i] = DynamicLatexAlignmentMenu.this.createItem(cmd.getIcon(), cmd.getTitle(), new SystemPasteCommand(cmd.getText()));
		  }
